  ...
  try {
    ...
    if (file.size > MAX_SIZE) {
      return NextResponse.json(
        { error: `Logo must not exceed 500KB (yours is ${(file.size / 1024).toFixed(1)}KB)` },
        { status: 400 }
      );
    }
    ...
    const path = `logos/${randomUUID()}.${extension}`;
    ...
    if (uploadError) {
      console.error("Failed to upload sponsor logo:", uploadError);
      return NextResponse.json(
